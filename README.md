...
目前版本：**v0.5.32 Pages Origin Continuity**
...
`v0.5.32` 修復既有 GitHub Pages origin 的舊 Flutter Service Worker 退休順序。正式入口會先解除本 App scope 的舊 worker，必要時重載一次，再以原 `life_maintenance` Drift database 啟動；不清除網站資料、不更換資料庫名稱。
