...
目前版本：**v0.5.38 UI v2 Design System & Device UX Fixes**
...
`v0.5.38` 建立集中式 UI v2 色彩、字級、間距、圓角、陰影與動態 Token，以及共用按鈕、操作卡與精簡頁首；設計系統先限縮套用於新增入口與正式表單。生活項目表單現在可在沒有分類時進入既有分類建立流程，鍵盤不再遮住儲存操作，長表單可捲至底部並正確承接手機 SafeArea。本版維持所有欄位、正式 Repository 資料流程與五入口不變。
