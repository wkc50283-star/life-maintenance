...
目前版本：**v0.5.30 Device Validation Baseline**
...
`v0.5.30` 已建立正式 Device Validation Baseline。安裝、原地升級、冷啟動、背景切換、強制關閉重啟、資料持久化與版本相容都有可重跑 Checklist；自動 Gate 鎖定平台識別、Drift 檔案重啟與完整性，但不會把 simulator 或 build success 冒充為實體裝置簽核。
