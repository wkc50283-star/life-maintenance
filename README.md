...
目前版本：**v0.5.39 App Shell & Life Overview UI v2 Motion**
...
`v0.5.39` 將既有 UI v2 Design System 套用至正式 App Shell 與生活總覽，保留五個入口、所有正式資料投影與操作流程。首頁提供平靜空狀態及通往既有「新增」入口的快速新增；Tab、區塊、卡片與按鈕加入 160～280ms 的第一階段回饋，並在系統要求減少動態效果時停用裝飾動畫。
