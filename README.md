...
目前版本：**v0.5.31 Web Long-term Validation**
...
`v0.5.31` 已建立正式 Web 長期使用驗收基線。重新整理、關閉分頁重開、瀏覽器重啟、背景恢復、Drift 持久化與 GitHub Pages 部署都有可重跑 Checklist；Pages build 關閉已棄用的 Flutter Service Worker，避免長期部署混用不同世代的 Flutter、Drift worker 與 WASM 資產。
